...
// Normalizer compartido con la ruta AdminOrderDetail (mismo contrato que la app móvil)
export function normalizeOrderHistoryResponse(
  body: unknown,
): OrderStatusHistory[] {
  if (Array.isArray(body)) return body as OrderStatusHistory[];
  if (isWrappedData(body) && Array.isArray(body.data)) {
    return body.data as OrderStatusHistory[];
  }
  return [];
}
