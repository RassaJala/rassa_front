...
const adminNav: NavItem[] = [
  ...
  { key: 'categories', label: 'Categorías', icon: '📁', path: '/admin/categorias' },
...
const roleNavMap: Record<string, NavItem[]> = {
  admin: adminNav,
  agricultor: [
    { key: 'products', label: 'Productos', icon: '🥬', path: '/agricultor/productos' },
    { key: 'orders', label: 'Pedidos', icon: '📦', path: '/agricultor/pedidos' },
  ],
  vendedor: [
...
export function Sidebar({ role }: { role: string }) {
  ...
  return (
    <aside style={{
      width: 260, minHeight: '100vh', height: '100vh', position: 'sticky', top: 0,
      ...
      <a href="/admin" style={{
        ...
        padding: '0 20px 24px', borderBottom: `1px solid ${borderColor}`,
        ...
        <span style={{
          ...
          <h1 style={{ fontSize: 18, fontWeight: 700, letterSpacing: '-0.01em', color: fg }}>RASSA-JALA</h1>
      </a>
        ...
        {items.map((item) => (
          <NavLink
            ...
            style={({ isActive }) => ({
              ...
              textDecoration: 'none', color: isActive ? brand : muted,
      ...
      {/* Footer */}
      <div style={{
        padding: '16px 20px', borderTop: `1px solid ${borderColor}`,
        display: 'flex', alignItems: 'center', gap: 10,
      }}>
        <div style={{
          width: 36, height: 36, borderRadius: 10,
          background: '#DE393A', color: '#fff', display: 'grid',
          placeItems: 'center', fontWeight: 600, fontSize: 14,
        }}>AD</div>
