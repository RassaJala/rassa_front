  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <h2 style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.01em', color: fg }}>Gestión de unidades</h2>
        <button onClick={startNew} style={{ ...btnStyle, background: coral, color: '#fff' }}>＋ Nueva unidad</button>
      </div>
      ...
      {tab === 'list' && (
        <div style={{ background: surface, borderRadius: 16, border: `1px solid ${border}`, overflow: 'hidden' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: `1px solid ${border}`, flexWrap: 'wrap', gap: 8 }}>
            <span style={{ fontSize: 14, fontWeight: 600, color: fg }}>{items.length} unidades</span>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={4} style={{ textAlign: 'center', padding: '48px 24px', color: muted, fontSize: 14 }}>No hay unidades</td></tr>
                ) : filtered.map((item) => (
                  <tr key={item.id} style={{ background: surface }}>
                    ...
                    <td style={{ padding: '14px 20px', fontSize: 14, borderBottom: `1px solid ${border}`, color: muted }}>{item.abreviatura}</td>
      ...
      {tab === 'form' && (
        <div style={{ background: surface, borderRadius: 16, border: `1px solid ${border}`, overflow: 'hidden' }}>
          <div style={{ padding: '20px 24px', borderBottom: `1px solid ${border}` }}>
            <span style={{ fontSize: 16, fontWeight: 600, color: fg }}>{editId ? 'Editar unidad' : 'Nueva unidad'}</span>
          </div>
      ...
      {delTarget && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, backdropFilter: 'blur(4px)' }}
          ...
          <div style={{ background: surface, borderRadius: 20, padding: 28, maxWidth: 440, width: '90%', border: `1px solid ${border}`, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}
            ...
            <h3 style={{ fontSize: 18, fontWeight: 700, color: fg, marginBottom: 8 }}>¿Eliminar unidad?</h3>
            <p style={{ fontSize: 14, color: muted, marginBottom: 20 }}>Vas a eliminar "{delTarget.nombre} ({delTarget.abreviatura})". Esta acción no se puede deshacer.</p>
