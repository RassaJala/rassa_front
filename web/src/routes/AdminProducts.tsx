...
const initialData: Product[] = [
  ...
  { id: 3, nombre: 'Café especial', categoria: 'Grano', precio: 180, stock: 28, unidad: 'kg', descripcion: 'Café de altura, tostado medio.', estado: true },
  { id: 4, nombre: 'Cebolla larga', categoria: 'Verdura', precio: 18, stock: 200, unidad: 'kg', descripcion: 'Cebolla larga fresca de la sabana.', estado: true },
  { id: 5, nombre: 'Maíz criollo', categoria: 'Grano', precio: 25, stock: 150, unidad: 'kg', descripcion: 'Maíz amarillo para arepas.', estado: true },
  { id: 6, nombre: 'Lechuga romana', categoria: 'Verdura', precio: 15, stock: 60, unidad: 'unidad', descripcion: 'Lechuga romana hidropónica.', estado: false },
];

const catEmoji: Record<string, string> = { Fruta: '🥑', Verdura: '🥬', Grano: '🌾', Otro: '📦' };
...
const unitLabels: Record<string, string> = { kg: 'kg', unidad: 'unid.', lb: 'lb', arroba: '@' };
...
export function AdminProducts() {
  ...
  function startNew() {
    setEditId(null);
    setForm({ nombre: '', categoria: '', precio: '', stock: '', unidad: 'kg', descripcion: '' });
    setTab('form');
  }

  function startEdit(item: Product) {
    setEditId(item.id);
    setForm({ nombre: item.nombre, categoria: item.categoria, precio: String(item.precio), stock: String(item.stock), unidad: item.unidad, descripcion: item.descripcion });
    setTab('form');
  }
  ...
  function toggleStatus(item: Product) {
    setItems((prev) => prev.map((i) => (i.id === item.id ? { ...i, estado: !i.estado } : i)));
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <h2 style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.01em', color: fg }}>Gestión de productos</h2>
        <button onClick={startNew} style={{ ...btnStyle, background: coral, color: '#fff' }}>＋ Nuevo producto</button>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: 2, background: border, borderRadius: 12, padding: 3, marginBottom: 20, width: 'fit-content' }}>
        {['list', 'form'].map((t) => (
          <button key={t} onClick={() => t === 'form' ? startNew() : setTab('list')} style={{
            padding: '8px 20px', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer',
            background: tab === t ? surface : 'transparent', color: tab === t ? fg : muted,
            boxShadow: tab === t ? '0 1px 3px rgba(0,0,0,0.06)' : 'none',
          }}>{t === 'list' ? '📋 Lista de productos' : '➕ Agregar producto'}</button>
        ))}
      </div>
      ...
      {tab === 'list' && (
        <div style={{ background: surface, borderRadius: 16, border: `1px solid ${border}`, overflow: 'hidden' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: `1px solid ${border}`, flexWrap: 'wrap', gap: 8 }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['Producto', 'Categoría', 'Precio', 'Stock', 'Estado', 'Acciones'].map((h) => (
                    <th key={h} style={{
                      textAlign: 'left', fontSize: 11, color: muted, textTransform: 'uppercase',
                      letterSpacing: '0.08em', fontWeight: 600, padding: '12px 20px',
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={6} style={{ textAlign: 'center', padding: '48px 24px', color: muted, fontSize: 14 }}>No hay productos</td></tr>
                ) : filtered.map((item) => {
                  const catBg = isDark ? '#1C2D22' : (catClass[item.categoria]?.bg ?? '#D0D8E8');
                  const catColor = isDark ? '#4A8A63' : (catClass[item.categoria]?.color ?? '#4A5A7A');
                  return (
                    <tr key={item.id}>
                      <td style={{ padding: '14px 20px', fontSize: 14, borderBottom: `1px solid ${border}`, fontWeight: 600, color: fg }}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span>{catEmoji[item.categoria] ?? '📦'} {item.nombre}</span>
                        </span>
                      </td>
                      <td style={{ padding: '14px 20px', borderBottom: `1px solid ${border}` }}>
                        <span style={{ fontSize: 12, padding: '3px 10px', borderRadius: 6, fontWeight: 600, background: catBg, color: catColor }}>{item.categoria}</span>
                      </td>
                      <td style={{ padding: '14px 20px', fontSize: 14, borderBottom: `1px solid ${border}`, color: fg }}>${item.precio} / {unitLabels[item.unidad] ?? item.unidad}</td>
                      <td style={{ padding: '14px 20px', fontSize: 14, borderBottom: `1px solid ${border}`, color: muted }}>{item.stock} {unitLabels[item.unidad] ?? item.unidad}</td>
                      <td style={{ padding: '14px 20px', borderBottom: `1px solid ${border}` }}>
                        <span style={{
                          fontSize: 12, fontWeight: 600, padding: '3px 10px', borderRadius: 6,
                          background: item.estado ? (isDark ? 'rgba(74,138,99,0.15)' : 'rgba(36,86,60,0.07)') : (isDark ? 'rgba(212,160,32,0.12)' : 'rgba(242,169,0,0.1)'),
                          color: item.estado ? brand : '#F2A900',
                        }}>{item.estado ? 'Activo' : 'Inactivo'}</span>
                      </td>
                      <td style={{ padding: '14px 20px', borderBottom: `1px solid ${border}` }}>
                        <div style={{ display: 'flex', gap: 4 }}>
                          <button onClick={() => toggleStatus(item)} aria-label={item.estado ? 'Desactivar' : 'Activar'} style={{
                            width: 32, height: 32, borderRadius: 8, border: `1px solid ${border}`,
                            background: surface, cursor: 'pointer', fontSize: 14, display: 'grid', placeItems: 'center', color: fg,
                          }}>{item.estado ? '⏸' : '▶️'}</button>
                          <button onClick={() => startEdit(item)} aria-label="Editar" style={{
                            width: 32, height: 32, borderRadius: 8, border: `1px solid ${border}`,
                            background: surface, cursor: 'pointer', fontSize: 14, display: 'grid', placeItems: 'center', color: fg,
                          }}>✏️</button>
                          <button onClick={() => setDelTarget(item)} aria-label="Eliminar" style={{
                            width: 32, height: 32, borderRadius: 8, border: `1px solid ${border}`,
                            background: surface, cursor: 'pointer', fontSize: 14, display: 'grid', placeItems: 'center', color: fg,
                          }}>🗑️</button>
                        </div>
      ...
      {tab === 'form' && (
        <div style={{ background: surface, borderRadius: 16, border: `1px solid ${border}`, overflow: 'hidden' }}>
          <div style={{ padding: '20px 24px', borderBottom: `1px solid ${border}` }}>
            <span style={{ fontSize: 16, fontWeight: 600, color: fg }}>{editId ? 'Editar producto' : 'Nuevo producto'}</span>
          </div>
          <form onSubmit={handleSave} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, padding: 24 }}>
      ...
      {delTarget && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, backdropFilter: 'blur(4px)' }}
          ...
          <div style={{ background: surface, borderRadius: 20, padding: 28, maxWidth: 440, width: '90%', border: `1px solid ${border}`, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}
            onClick={(e) => e.stopPropagation()}>
            <h3 style={{ fontSize: 18, fontWeight: 700, color: fg, marginBottom: 8 }}>¿Eliminar producto?</h3>
            <p style={{ fontSize: 14, color: muted, marginBottom: 20 }}>Vas a eliminar "{delTarget.nombre}". Esta acción no se puede deshacer.</p>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button onClick={() => setDelTarget(null)}
                style={{ height: 32, padding: '0 12px', borderRadius: 8, border: `1.5px solid ${border}`, background: 'transparent', color: fg, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Cancelar</button>
              <button onClick={handleDelete}
                style={{ height: 32, padding: '0 12px', borderRadius: 8, border: '1.5px solid #DE393A', background: 'transparent', color: '#DE393A', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Eliminar</button>
            </div>
          </div>
        </div>
      )}
