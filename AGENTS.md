...
4. `src/styles/global.css` — imported at top of `App.tsx`.
...
- **React Navigation 7** — `src/navigation/AppNavigator.tsx` handles role-based routing.
- **TanStack Query 5** — for server state (configured in `App.tsx`).
- **React Native Paper 5** — UI component library (Material Design).
