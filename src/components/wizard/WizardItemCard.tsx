  ...
  return (
    <View className="mb-3 rounded-xl border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
      <View className="mb-2 flex-row items-center justify-between">
        <Text className="flex-1 pr-2 text-sm font-medium text-gray-900 dark:text-gray-100">
          {nombre}
        </Text>
        <Pressable
      ...
      {/* Photo + fields (compact horizontal layout like web) */}
      <View className="flex-row gap-3">
        <View className="shrink-0">
          <Pressable
            onPress={() => onPickImage(item.tempId)}
            className={`h-20 w-20 items-center justify-center overflow-hidden rounded-lg border-2 border-dashed ${
              validation?.foto
          ...
          >
            {item.foto ? (
              ...
              <Text className="px-1 text-center text-[10px] text-gray-400 dark:text-gray-500">
                Toca para foto
              </Text>
            )}
          </Pressable>
          {validation?.foto ? (
            <Text className="mt-0.5 max-w-20 text-xs text-red-500">
              {validation.foto}
            </Text>
          ) : null}
        </View>
            ...
            <View className="flex-row flex-wrap gap-1.5">
              {unidades.map((u) => (
                <Pressable
                  key={u.id_unidad}
                  onPress={() =>
                    onUpdate(item.tempId, 'fk_unidad', u.id_unidad)
                  }
                  ...
                  accessibilityRole="button"
                  accessibilityState={{
                    selected: item.fk_unidad === u.id_unidad,
                  }}
                >
                  <Text
