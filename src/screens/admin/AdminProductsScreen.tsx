...
function CategoryPickerModal({
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <Pressable style={{ flex: 1, backgroundColor: overlayBg, justifyContent: 'center', padding: 40 }} onPress={onClose}>
        <View style={{ backgroundColor: surface, borderRadius: 20, padding: 16, gap: 4 }}>
          ...
          {categories.map((cat) => (
            <TouchableOpacity key={cat} onPress={() => { onSelect(cat); onClose(); }} activeOpacity={0.7}
              style={{ paddingVertical: 14, paddingHorizontal: 8, borderRadius: 12, backgroundColor: selected === cat ? accentBg : transparentBg }}>
              <Text style={{ fontSize: 16, color: fg, fontWeight: selected === cat ? '600' : '400' }}>{cat}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </Pressable>
...
function ToggleConfirmModal({
  ...
  return (
    <Modal visible={target !== null} transparent animationType="slide" onRequestClose={onClose}>
      <Pressable style={{ flex: 1, backgroundColor: overlayBg }} onPress={onClose} />
      <View style={{ backgroundColor: surface, borderRadius: 24, padding: 24, paddingBottom: 34, marginTop: 'auto' }}>
        <View style={{ alignItems: 'center', marginBottom: 16 }}>
          <View style={{ width: 56, height: 56, borderRadius: 28, alignItems: 'center', justifyContent: 'center', backgroundColor: iconBg, marginBottom: 12 }}>
            <MaterialCommunityIcons name={target?.estado ? 'pause-circle-outline' : 'play-circle-outline'} size={26} color={errorColor} />
          </View>
          <Text style={{ fontSize: 17, fontWeight: '700', color: fg, textAlign: 'center' }}>
            {target?.estado ? `Desactivar "${target?.name}"?` : `Activar "${target?.name}"?`}
          </Text>
          <Text style={{ fontSize: 14, color: muted, marginTop: 6, textAlign: 'center' }}>
            {target?.estado ? 'El producto se moverá a la papelera.' : 'El producto volverá a estar activo.'}
          </Text>
        </View>
        <View style={{ gap: 10 }}>
          <TouchableOpacity onPress={() => { if (target) onConfirm(target); onClose(); }} activeOpacity={0.8}
            style={{ height: 50, borderRadius: 14, backgroundColor: errorColor, alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ fontSize: 16, fontWeight: '600', color: iconWhite }}>{target?.estado ? 'Desactivar' : 'Activar'}</Text>
          </TouchableOpacity>
...
function DeleteConfirmModal({
  target, onPermanentDelete, onToggleStatus, onClose, surface, fg, muted, iconWhite, errorColor, border, iconBg,
}: {
  target: Product | null; onPermanentDelete: (t: Product) => void; onToggleStatus: (t: Product) => void; onClose: () => void;
        ...
        <View style={{ gap: 10 }}>
          <TouchableOpacity onPress={() => {
            ...
            <Text style={{ fontSize: 16, fontWeight: '600', color: iconWhite }}>
              {target?.estado === false ? 'Eliminar permanentemente' : 'Desactivar'}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={onClose} activeOpacity={0.8}
            style={{ height: 44, borderRadius: 14, borderWidth: 1.5, borderColor: border, alignItems: 'center', justifyContent: 'center' }}>
...
export default function AdminProductsScreen(): React.JSX.Element {
  ...
  function renderCard(item: Product) {
    ...
    const iconName = isTrash ? ('delete-restore' as const) : ('package-variant' as const);
    ...
    return (
      <View style={{ backgroundColor: surface, borderRadius: 16, borderWidth: 1, borderColor: border, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 14 }}>
        <View style={{ width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center', backgroundColor: isTrash ? trashBg : accentBg }}>
  ...
  function renderSegmentedControl() {
    ...
    return (
      <View style={{ paddingHorizontal: 20, paddingTop: 12, paddingBottom: 16 }}>
        <View style={{ flexDirection: 'row', backgroundColor: segBg, borderRadius: 10, padding: 3 }}>
          <TouchableOpacity onPress={() => { if (!isFormActive) return; switchToList(); }}
            style={{ flex: 1, paddingVertical: 8, borderRadius: 8, backgroundColor: isFormActive ? transparentBg : surface, alignItems: 'center' }} activeOpacity={0.7}>
          ...
          <TouchableOpacity onPress={() => { if (isFormActive) return; startNew(); }}
            style={{ flex: 1, paddingVertical: 8, borderRadius: 8, backgroundColor: isFormActive ? surface : transparentBg, alignItems: 'center' }} activeOpacity={0.7}>
