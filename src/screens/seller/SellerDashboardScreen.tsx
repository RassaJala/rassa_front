...
const mockOrders = [
  { id: "#ORD-001", buyer: "Carlos M.", products: "Manzanas x2", total: "$90", status: "Pendiente", date: "Hoy 09:30" },
  { id: "#ORD-002", buyer: "Lucía R.", products: "Lechugas x5", total: "$90", status: "Confirmado", date: "Hoy 08:15" },
  { id: "#ORD-003", buyer: "Pedro G.", products: "Tomates x3", total: "$96", status: "Enviado", date: "Ayer 17:40" },
  { id: "#ORD-004", buyer: "Ana T.", products: "Manzanas x1", total: "$45", status: "Entregado", date: "Ayer 10:20" },
];
    ...
    <View style={styles.root}>
      ...
      {!isMobile && (
        <View style={styles.sidebar}>
          ...
          {SECTIONS.map((s) => (
            <TouchableOpacity
              key={s}
              style={[styles.sidebarItem, activeSection === s && styles.sidebarActive]}
              onPress={() => setActiveSection(s)}
            >
              <Text style={[styles.sidebarText, activeSection === s && styles.sidebarTextActive]}>
                {s}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <ScrollView style={styles.main} contentContainerStyle={styles.mainContent}>
        ...
        {isMobile && (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.mobileNav}>
            {SECTIONS.map((s) => (
              <TouchableOpacity
                key={s}
                style={[styles.mobileNavItem, activeSection === s && styles.mobileNavItemActive]}
                onPress={() => setActiveSection(s)}
              >
                <Text style={[styles.mobileNavText, activeSection === s && styles.mobileNavTextActive]}>
                  {s}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View style={styles.topBar}>
          ...
          {!isMobile && <Text style={styles.pageDate}>Viernes, 27 Jun 2026</Text>}
        </View>
        ...
        {activeSection === "Dashboard" && (
          <>
            ...
            <View style={styles.statsGrid}>
              {[
                { label: "Órdenes hoy", value: "12", icon: "📦", color: "#f59e0b" },
                { label: "Ingresos hoy", value: "$1,240", icon: "💰", color: "#16a34a" },
                { label: "Pendientes", value: "5", icon: "🕐", color: "#f97316" },
                { label: "Clientes mes", value: "89", icon: "👥", color: "#6366f1" },
              ].map((s) => (
              ...
              <View style={styles.ordersTable}>
                ...
                {mockOrders.map((o) => (
                  <View key={o.id} style={styles.tableRow}>
                    <Text style={[styles.tableCell, styles.orderId]}>{o.id}</Text>
                    <Text style={styles.tableCell}>{o.buyer}</Text>
                    <Text style={styles.tableCell}>{o.products}</Text>
                    <Text style={[styles.tableCell, styles.total]}>{o.total}</Text>
                    <View style={styles.tableCellStatus}>
                      <View style={[styles.statusBadge, { backgroundColor: STATUS_COLORS[o.status]?.bg }]}>
                        <Text style={[styles.statusText, { color: STATUS_COLORS[o.status]?.text }]}>
                          {o.status}
                        </Text>
                      </View>
                    </View>
        ...
        {activeSection !== "Dashboard" && (
          <View style={styles.placeholder}>
            ...
            <Text style={styles.placeholderText}>Conecta tu API para mostrar datos reales en esta sección.</Text>
          </View>
        )}
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  roleBadge: { backgroundColor: "#f59e0b", borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3, alignSelf: "flex-start" },
  roleBadgeText: { color: "#fff", fontSize: 11, fontWeight: "700" },
  sidebarItem: { paddingHorizontal: 20, paddingVertical: 12 },
  sidebarActive: { backgroundColor: "#1e293b", borderLeftWidth: 3, borderLeftColor: "#f59e0b" },
  ...
  avatar: { width: 36, height: 36, borderRadius: 18, backgroundColor: "#f59e0b", justifyContent: "center", alignItems: "center" },
  avatarText: { color: "#fff", fontWeight: "800", fontSize: 13 },
  userName: { color: "#94a3b8", fontSize: 13, flex: 1 },
  logoutBtn: { backgroundColor: "#ef4444", borderRadius: 8, paddingVertical: 10, alignItems: "center" },
  ...
  ordersTable: { backgroundColor: "#fff", borderRadius: 14, overflow: "hidden", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 3 },
  tableHead: { flexDirection: "row", backgroundColor: "#f1f5f9", paddingHorizontal: 12, paddingVertical: 10 },
  tableHeadCell: { width: 90, fontWeight: "700", fontSize: 11, color: "#64748b", textTransform: "uppercase" },
  tableRow: { flexDirection: "row", paddingHorizontal: 12, paddingVertical: 10, borderTopWidth: 1, borderTopColor: "#f1f5f9", alignItems: "center" },
