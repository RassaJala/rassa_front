...
describe('OrderHistoryScreen', () => {
  ...
  it('muestra badge Con mermas cuando un pedido tiene mermas', () => {
    const ordersWithMermas = [
      {
        ...mockOrders[0],
        tiene_mermas: true,
      },
    ...
    expect(getAllByText('Con mermas')).toHaveLength(1);
  });

  it('filtra por mermas al activar el filtro', () => {
    const ordersWithMermas = [
      {
        ...mockOrders[0],
        tiene_mermas: true,
    ...
    fireEvent.press(getByText('Solo con mermas'));

    expect(queryByText('#2')).toBeNull();
  });

  it('muestra empty state cuando el filtro no tiene resultados', () => {
    const ordersWithMermas = [
      {
        ...mockOrders[0],
        tiene_mermas: false,
    ...
    expect(getByText('No hay pedidos con mermas')).toBeTruthy();
  });
});
