...
describe('withTimeout', () => {
  ...
  it('rejects with timeout error if promise is too slow', async () => {
    ...
    await expect(promise).rejects.toThrow('tardó más de 1s');
  });
...
const { usePublicationWizard } = require('@/hooks/usePublicationWizard') as typeof import('@/hooks/usePublicationWizard');
...
const sampleProducto: Producto = {
  ...
  precio: '1500',
  stock: 20,
  categoria: { id_categoria: 1, nombre: 'Verdura', descripcion: 'Verduras frescas' },
...
describe('usePublicationWizard hook', () => {
  ...
  it('validateItems returns true when all items are valid', () => {
    ...
    act(() => result.current.updateItem(tempId!, 'foto', 'https://example.com/foto.jpg'));
